...
A token is used to authorize a user to access different data that's provided by the server.

- [ ] What steps can you take in your web apps to keep your data secure?

you can create a private route to set  certain components of the data to be visible with a token, also some tokens time-out so that's another way of prevention.

- [ ] Describe how web servers work.

Web servers send, manage and store data based on client interaction and authorization.

- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with 
APIs/Servers.
Create( .post, .put) Read (.get) Update(.put, .post), Delete(.delete)
